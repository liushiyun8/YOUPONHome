...
import com.youpon.home1.comm.tools.XTGlobals;
...
public class DeviceManage {
    ...
    public static ConcurrentHashMap<String, Device> currentdeviceMap = new ConcurrentHashMap<String, Device>();
        ...
        while (iter.hasNext()) {
            ...
            try {
                ...
                if (xdev != null) {
                    ...
                    if (!obj.isNull(ROLE)) {
                        device.setRole(obj.getInt(ROLE));
                    }
                    if (!obj.isNull(ROOM)) {
    ...
    public synchronized void addCurrentdev(Device dev){
        currentdeviceMap.put(dev.getMacAddress(), dev);
    }

    public synchronized void removeCurrentdev(Device dev){
        currentdeviceMap.remove(dev.getMacAddress());
    }

    public synchronized void removeCurrentXdev(XDevice dev){
        currentdeviceMap.remove(dev.getMacAddress());
    }

    public synchronized Device getCurrentdev(Device dev){
        return currentdeviceMap.get(dev.getMacAddress());
    }

    public synchronized Device getCurrentdev(XDevice dev){
        return currentdeviceMap.get(dev.getMacAddress());
    }
    ...
    public synchronized List<Device> getCurrentdev(){
        List<Device> list=new ArrayList<>();
        list.addAll(currentdeviceMap.values());
        return list;
    }
    ...
    public void saveDevice(Device device) {
        ...
        if(device.getName()!=null){
            ...
            if(device.getRole()!=0){
                jo.put(ROLE, device.getRole());
            }
            if(device.getRoom()!=null){
                jo.put(ROOM, device.getRoom());
            }
        }
        catch (JSONException e) {
            e.printStackTrace();
